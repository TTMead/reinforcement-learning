...
from batch_helpers import batchify_obs, batchify, unbatchify, clip_actions, save_model
    ...
    try:
        for update in range(1, args.num_updates + 1):
            ...
            if (global_step > (num_model_saves * args.save_period)):
                save_model(agent, run_name, args.exp_name + "_" + str(num_model_saves))
                num_model_saves += 1
    except:
        print("Cancelling training run early due to exception:", traceback.print_exc(), "\n")
        pass

    save_model(agent, run_name, args.exp_name)

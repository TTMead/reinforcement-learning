...
class JestelNetwork(nn.Module):
    def __init__(self, output_size):
        ...
        self.fire_stream = nn.Sequential(
            nn.Linear(Agent.stack_size() * 16, 48 * 4),
            nn.ReLU()
        )
    ...
    def forward(self, raw_observation):
        ...
        o_f = observation[:,:,272:288].reshape(batch_size, -1)
        o_a = observation[:,:,288:297].reshape(batch_size, -1)
        ...
        assert (o_f.shape == (batch_size, Agent.stack_size() * 16)), "Fire observation should have a shape of (batch_size, " + str(Agent.stack_size() * 16) + "), received " + str(o_f.shape)
...
class Agent(nn.Module):
    ...
    def get_observation_range():
        ...
        # O_f (fire state) (rel_x, rel_y, dist, intensity) ranges
        num_fires = 4
        for i in range(num_fires):
            ranges = np.vstack([ranges, [-20, 20]])
            ranges = np.vstack([ranges, [-20, 20]])
            ranges = np.vstack([ranges, [0, 40]])
            ranges = np.vstack([ranges, [0, 8]])
